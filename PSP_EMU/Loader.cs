...
//ORIGINAL LINE: public pspsharp.HLE.kernel.types.SceModule LoadModule(String pspfilename, ByteBuffer f, int baseAddress, int mpidText, int mpidData, bool analyzeOnly, bool allocMem, bool fromSyscall) throws java.io.IOException
		public virtual SceModule LoadModule(string pspfilename, ByteBuffer f, int baseAddress, int mpidText, int mpidData, bool analyzeOnly, bool allocMem, bool fromSyscall)
		{
			...
			do
			{
				...
				if (module.psf != null)
				{
					Console.WriteLine(string.Format("PBP meta data:{0}{1}", Environment.NewLine, module.psf));
		...
		private bool LoadELF(ByteBuffer f, SceModule module, int baseAddress, bool analyzeOnly, bool allocMem, bool fromSyscall)
		{
			...
			if (elf.Header.Valid)
			{
				...
				else if (elf.Header.requiresRelocation())
				{
					...
					Console.WriteLine("Loader: Relocation required (ELF)");
				}
				else
				{
...
//ORIGINAL LINE: private bool LoadUNK(ByteBuffer f, pspsharp.HLE.kernel.types.SceModule module, int baseAddress, bool analyzeOnly, bool allocMem, bool fromSyscall) throws java.io.IOException
		private bool LoadUNK(ByteBuffer f, SceModule module, int baseAddress, bool analyzeOnly, bool allocMem, bool fromSyscall)
		{
			...
			{ // CSO
				Console.WriteLine("This is not an executable file!");
				Console.WriteLine("Try using the Load UMD menu item");
			}
			...
			else
			{
				...
				if (!handled)
				{
					...
					Console.WriteLine(string.Format("File magic {0:X2} {1:X2} {2:X2} {3:X2}", m0, m1, m2, m3));
					//if (log.DebugEnabled)
					{
...
//ORIGINAL LINE: private void LoadELFModuleInfo(ByteBuffer f, pspsharp.HLE.kernel.types.SceModule module, int baseAddress, pspsharp.format.Elf32 elf, int elfOffset, bool analyzeOnly) throws java.io.IOException
		private void LoadELFModuleInfo(ByteBuffer f, SceModule module, int baseAddress, Elf32 elf, int elfOffset, bool analyzeOnly)
		{
			...
			if (!analyzeOnly)
			{
				if (log.InfoEnabled)
				{
					Console.WriteLine(string.Format("Found ModuleInfo at 0x{0:X8}, name:'{1}', version: {2:X2}{3:X2}, attr: 0x{4:X8}, gp: 0x{5:X8}", moduleInfoAddr, module.modname, module.version[1], module.version[0], module.attribute, module.gp_value));
				}
...
//JAVA TO C# CONVERTER TODO TASK: .NET enumerators are read-only:
			...
			Console.WriteLine(numberofmappedNIDS + " NIDS mapped");
			if (numberoffailedNIDS > 0)
			{
				Console.WriteLine(numberoffailedNIDS + " remaining unmapped NIDS");
			}
		}
...
//ORIGINAL LINE: private void LoadELFImports(pspsharp.HLE.kernel.types.SceModule module) throws java.io.IOException
		private void LoadELFImports(SceModule module)
		{
			...
			if (module.unresolvedImports.Count > 0)
			{
				if (log.InfoEnabled)
				{
					Console.WriteLine(string.Format("Found {0:D} unresolved imports", module.unresolvedImports.Count));
				}
			}
		}
...
//ORIGINAL LINE: private void LoadELFExports(pspsharp.HLE.kernel.types.SceModule module) throws java.io.IOException
		private void LoadELFExports(SceModule module)
		{
			...
			if (entCount > 0)
			{
				if (log.InfoEnabled)
				{
					Console.WriteLine(string.Format("Found {0:D} exports", entCount));
				}
			}
		}
