...
//ORIGINAL LINE: public pspsharp.HLE.kernel.types.SceModule LoadModule(String pspfilename, ByteBuffer f, int baseAddress, int mpidText, int mpidData, bool analyzeOnly, bool allocMem, bool fromSyscall) throws java.io.IOException
		public virtual SceModule LoadModule(string pspfilename, ByteBuffer f, int baseAddress, int mpidText, int mpidData, bool analyzeOnly, bool allocMem, bool fromSyscall)
		{
			...
			{
				System.Console.WriteLine("LoadModule: no data.");
				return module;
			}
			...
			do
			{
				...
				if (module.psf != null)
				{
					System.Console.WriteLine(string.Format("PBP meta data:{0}{1}", Environment.NewLine, module.psf));
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: private bool LoadPBP(ByteBuffer f, pspsharp.HLE.kernel.types.SceModule module, int baseAddress, bool analyzeOnly, bool allocMem, bool fromSyscall) throws java.io.IOException
		...
		/// <returns> true on success </returns>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: private bool LoadSPRX(ByteBuffer f, pspsharp.HLE.kernel.types.SceModule module, int baseAddress, bool analyzeOnly, bool allocMem, bool fromSyscall) throws java.io.IOException
		private bool LoadSPRX(ByteBuffer f, SceModule module, int baseAddress, bool analyzeOnly, bool allocMem, bool fromSyscall)
		{
			...
			{
				System.Console.WriteLine("Encrypted file detected! (.PSPEDAT)");
		...
		/// <returns> true on success </returns>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: private bool LoadSCE(ByteBuffer f, pspsharp.HLE.kernel.types.SceModule module, int baseAddress, bool analyzeOnly, bool allocMem, bool fromSyscall) throws java.io.IOException
		private bool LoadSCE(ByteBuffer f, SceModule module, int baseAddress, bool analyzeOnly, bool allocMem, bool fromSyscall)
		{
			...
			if (magic == SCE_MAGIC)
			{
				...
				System.Console.WriteLine("Encrypted file not supported! (~SCE)");
				return true;
			}
			// Not a valid PSP
			return false;
		...
		/// <returns> true on success </returns>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: private bool LoadPSP(ByteBuffer f, pspsharp.HLE.kernel.types.SceModule module, int baseAddress, bool analyzeOnly, bool allocMem, bool fromSyscall) throws java.io.IOException
		private bool LoadPSP(ByteBuffer f, SceModule module, int baseAddress, bool analyzeOnly, bool allocMem, bool fromSyscall)
		{
			...
			//if (log.DebugEnabled)
			{
				System.Console.WriteLine(string.Format("Called crypto engine for PRX (duration={0:D} ms)", end - start));
			}
		...
		/// <returns> true on success </returns>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: private bool LoadELF(ByteBuffer f, pspsharp.HLE.kernel.types.SceModule module, int baseAddress, bool analyzeOnly, bool allocMem, bool fromSyscall) throws java.io.IOException
		private bool LoadELF(ByteBuffer f, SceModule module, int baseAddress, bool analyzeOnly, bool allocMem, bool fromSyscall)
		{
			...
			if (elf.Header.Valid)
			{
				...
				{
					System.Console.WriteLine("Loader NOT a MIPS executable");
					return false;
				}
				...
				{
					System.Console.WriteLine("Loader: Relocation required (PRX)");
				...
				else if (elf.Header.requiresRelocation())
				{
					...
					System.Console.WriteLine("Loader: Relocation required (ELF)");
				}
				else
				{
					...
					{
						System.Console.WriteLine("Loader: Probably trying to load PBP ELF while another PBP ELF is already loaded");
					}
				...
				{
					System.Console.WriteLine(string.Format("Incorrect ELF module address: loadAddressLow=0x{0:X8}, loadAddressHigh=0x{1:X8}", module.loadAddressLow, module.loadAddressHigh));
			...
			// Not a valid ELF
			System.Console.WriteLine("Loader: Not a ELF");
			return false;
		}
		...
		/// <returns> true on success  </returns>
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: private bool LoadUNK(ByteBuffer f, pspsharp.HLE.kernel.types.SceModule module, int baseAddress, bool analyzeOnly, bool allocMem, bool fromSyscall) throws java.io.IOException
		private bool LoadUNK(ByteBuffer f, SceModule module, int baseAddress, bool analyzeOnly, bool allocMem, bool fromSyscall)
		{
			...
			{ // CSO
				System.Console.WriteLine("This is not an executable file!");
				System.Console.WriteLine("Try using the Load UMD menu item");
			}
			...
			else
			{
				...
				if (!handled)
				{
					System.Console.WriteLine("Unrecognized file format");
					System.Console.WriteLine(string.Format("File magic {0:X2} {1:X2} {2:X2} {3:X2}", m0, m1, m2, m3));
					//if (log.DebugEnabled)
					{
						...
						System.Console.WriteLine(string.Format("File header: {0}", Utilities.getMemoryDump(buffer, 0, buffer.Length)));
					}
				}
			}
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: private void LoadELFProgram(ByteBuffer f, pspsharp.HLE.kernel.types.SceModule module, int baseAddress, pspsharp.format.Elf32 elf, int elfOffset, bool analyzeOnly) throws java.io.IOException
		private void LoadELFProgram(ByteBuffer f, SceModule module, int baseAddress, Elf32 elf, int elfOffset, bool analyzeOnly)
		{
			...
			foreach (Elf32ProgramHeader phdr in programHeaderList)
			{
				...
				if (phdr.P_type == 0x00000001L)
				{
					...
					if (!Memory.isAddressGood(memOffset))
					{
						...
						if (!Memory.isAddressGood(memOffset))
						{
							System.Console.WriteLine(string.Format("Program header has invalid memory offset 0x{0:X8}!", memOffset));
						}
					}
					...
					//if (log.DebugEnabled)
					{
						System.Console.WriteLine(string.Format("PH#{0:D}: loading program {1:X8} - file {2:X8} - mem {3:X8}", i, memOffset, memOffset + fileLen, memOffset + memLen));
						System.Console.WriteLine(string.Format("PH#{0:D}:\n{1}", i, phdr));
					}
					...
					if (f.position() + fileLen > f.limit())
					{
						...
						System.Console.WriteLine(string.Format("PH#{0:D}: program overflow clamping len {1:X8} to {2:X8}", i, fileLen, newLen));
						fileLen = newLen;
					}
					if (!analyzeOnly)
					{
					...
					if (memOffset < module.loadAddressLow)
					{
						...
						//if (log.DebugEnabled)
						{
							System.Console.WriteLine(string.Format("PH#{0:D}: new loadAddressLow {1:X8}", i, module.loadAddressLow));
						}
					}
			...
			//if (log.DebugEnabled)
			{
				System.Console.WriteLine(string.Format("PH alloc consumption {0:X8} (mem {1:X8})", (module.loadAddressHigh - module.loadAddressLow), module.bss_size));
			}
		}
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: private void LoadELFSections(ByteBuffer f, pspsharp.HLE.kernel.types.SceModule module, int baseAddress, pspsharp.format.Elf32 elf, int elfOffset, bool analyzeOnly) throws java.io.IOException
					...
					switch (shdr.Sh_type)
					{
						...
						{ // 1
							...
							if (len == 0)
							{
								//if (log.DebugEnabled)
								{
									System.Console.WriteLine(string.Format("{0}: ignoring zero-Length type 1 section {1:X8}", shdr.Sh_namez, memOffset));
								}
							}
							else if (!Memory.isAddressGood(memOffset))
							{
								System.Console.WriteLine(string.Format("Section header (type 1) has invalid memory offset 0x{0:X8}!", memOffset));
							}
							else
							{
								// Update memory area consumed by the module
								if (memOffset < module.loadAddressLow)
								{
									System.Console.WriteLine(string.Format("{0}: section allocates more than program {1:X8} - {2:X8}", shdr.Sh_namez, memOffset, (memOffset + len)));
									module.loadAddressLow = memOffset;
								}
						...
						{ // 8
							...
							if (len == 0)
							{
								//if (log.DebugEnabled)
								{
									System.Console.WriteLine(string.Format("{0}: ignoring zero-Length type 8 section {1:X8}", shdr.Sh_namez, memOffset));
								}
							}
							else if (!Memory.isAddressGood(memOffset))
							{
								System.Console.WriteLine(string.Format("Section header (type 8) has invalid memory offset 0x{0:X8}!", memOffset));
							}
							else
							{
								//if (log.DebugEnabled)
								{
									System.Console.WriteLine(string.Format("{0}: clearing section {1:X8} - {2:X8} (len {3:X8})", shdr.Sh_namez, memOffset, (memOffset + len), len));
								}
								...
								if (memOffset < module.loadAddressLow)
								{
									...
									//if (log.DebugEnabled)
									{
										System.Console.WriteLine(string.Format("{0}: new loadAddressLow {1:X8} (+{2:X8})", shdr.Sh_namez, module.loadAddressLow, len));
									}
								}
								if (memOffset + len > module.loadAddressHigh)
								{
									...
									//if (log.DebugEnabled)
									{
										System.Console.WriteLine(string.Format("{0}: new loadAddressHigh {1:X8} (+{2:X8})", shdr.Sh_namez, module.loadAddressHigh, len));
									}
								}
							}
							break;
		...
		private void LoadELFReserveMemory(SceModule module)
		{
			...
			//if (log.DebugEnabled)
			{
				System.Console.WriteLine(string.Format("Reserving 0x{0:X} bytes at 0x{1:X8} for module '{2}'", module.loadAddressHigh - module.loadAddressLow, module.loadAddressLow, module.pspfilename));
			}
			...
			{
				System.Console.WriteLine(string.Format("Failed to properly reserve memory consumed by module {0} at address 0x{1:X8}, size 0x{2:X}: allocated {3}", module.modname, address, size, info));
			}
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: private void LoadELFModuleInfo(ByteBuffer f, pspsharp.HLE.kernel.types.SceModule module, int baseAddress, pspsharp.format.Elf32 elf, int elfOffset, bool analyzeOnly) throws java.io.IOException
		private void LoadELFModuleInfo(ByteBuffer f, SceModule module, int baseAddress, Elf32 elf, int elfOffset, bool analyzeOnly)
		{
			...
			if (!elf.Header.PRXDetected && shdr == null)
			{
				...
				{
					System.Console.WriteLine("ELF is not PRX, but has no section headers!");
					moduleInfoAddr = phdr.P_vaddr + (phdr.P_paddr & Memory.addressMask) - phdr.P_offset;
					System.Console.WriteLine("Manually locating ModuleInfo at address: 0x" + moduleInfoAddr.ToString("x"));
				}
			}
			...
			{
				System.Console.WriteLine("ModuleInfo not found!");
				return;
			}
			...
			if (!analyzeOnly)
			{
				if (log.InfoEnabled)
				{
					System.Console.WriteLine(string.Format("Found ModuleInfo at 0x{0:X8}, name:'{1}', version: {2:X2}{3:X2}, attr: 0x{4:X8}, gp: 0x{5:X8}", moduleInfoAddr, module.modname, module.version[1], module.version[0], module.attribute, module.gp_value));
				}

				if ((module.attribute & SceModule.PSP_MODULE_KERNEL) != 0)
				{
					System.Console.WriteLine("Kernel mode module detected");
				}
				if ((module.attribute & SceModule.PSP_MODULE_VSH) != 0)
				{
					System.Console.WriteLine("VSH mode module detected");
				}
			}
		}
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: private void relocateFromBuffer(ByteBuffer f, pspsharp.HLE.kernel.types.SceModule module, int baseAddress, pspsharp.format.Elf32 elf, int RelCount, bool pspRelocationFormat) throws java.io.IOException
		private void relocateFromBuffer(ByteBuffer f, SceModule module, int baseAddress, Elf32 elf, int RelCount, bool pspRelocationFormat)
		{
			...
			for (int i = 0; i < RelCount; i++)
			{
				...
				switch (R_TYPE)
				{
					...
					default:
						System.Console.WriteLine(string.Format("Unhandled relocation type {0:D} at 0x{1:X8}", R_TYPE, data_addr));
						break;
				}
...
//ORIGINAL LINE: sealed override int[] rebootTypeRemapping = new int[] { 0, 3, 6, 7, 1, 2, 4, 5 };
			...
			while (pos < end)
			{
				...
				if ((R_FLAG & 0x01) == 0)
				{
					...
					{
						System.Console.WriteLine("PH Relocation type 0x700000A1: Invalid size flag!");
						R_BASE = 0;
					}
				}
				else
				{ // Operate on segment address based on the relocation flag.
					...
					{
						System.Console.WriteLine("PH Relocation type 0x700000A1: Invalid relocation size flag!");
					}
					...
					{
						System.Console.WriteLine("PH Relocation type 0x700000A1: Invalid lo16 setup!");
					}
					else
					{
						System.Console.WriteLine("PH Relocation type 0x700000A1: Invalid lo16 setup!");
					...
					//if (log.DebugEnabled)
					{
						System.Console.WriteLine(string.Format("Relocation #{0:D} type={1:D}, Offset PH#{2:D}, Base Offset PH#{3:D}, Offset 0x{4:X8}", r, R_TYPE, OFS_BASE, ADDR_BASE, R_OFFSET));
					}
		...
		private void relocateFromHeaders(ByteBuffer f, SceModule module, int baseAddress, Elf32 elf, int elfOffset)
		{
			...
			foreach (Elf32ProgramHeader phdr in elf.ProgramHeaderList)
			{
				if (phdr.P_type == 0x700000A0L)
				{
					...
					//if (log.DebugEnabled)
					{
						System.Console.WriteLine(string.Format("PH#{0:D}: relocating {1:D} entries", i, RelCount));
					}
				...
				else if (phdr.P_type == 0x700000A1L)
				{
					//if (log.DebugEnabled)
					{
						System.Console.WriteLine(string.Format("Type 0x700000A1 PH#{0:D}: relocating A1 entries, size=0x{1:X}", i, phdr.P_filesz));
					}
					f.position(elfOffset + phdr.P_offset);
			...
			foreach (Elf32SectionHeader shdr in elf.SectionHeaderList)
			{
				if (mustRelocate(elf, shdr))
				{
					...
					//if (log.DebugEnabled)
					{
						System.Console.WriteLine(shdr.Sh_namez + ": relocating " + RelCount + " entries");
					}
...
//JAVA TO C# CONVERTER TODO TASK: .NET enumerators are read-only:
						...
						//if (log.DebugEnabled)
						{
							System.Console.WriteLine(string.Format("Mapped import at 0x{0:X8} to export at 0x{1:X8} [0x{2:X8}] (attempt {3:D})", importAddress, exportAddress, nid, module.importFixupAttempts));
						}
					}
					else if (nid == 0)
					{
						// Ignore patched nids
						System.Console.WriteLine(string.Format("Ignoring import at 0x{0:X8} [0x{1:X8}] (attempt {2:D})", importAddress, nid, module.importFixupAttempts));
					...
					else
					{
						...
						if (code >= 0)
						{
							...
							{
								System.Console.WriteLine(string.Format("Mapped import at 0x{0:X8} to syscall 0x{1:X5} [0x{2:X8}] (attempt {3:D})", importAddress, code, nid, module.importFixupAttempts));
							}
						}
						else
						{
							System.Console.WriteLine(string.Format("Failed to map import at 0x{0:X8} [0x{1:X8}] Module '{2}'(attempt {3:D})", importAddress, nid, moduleName, module.importFixupAttempts));
							numberoffailedNIDS++;
						}
					}
				}
			...
			System.Console.WriteLine(numberofmappedNIDS + " NIDS mapped");
			if (numberoffailedNIDS > 0)
			{
				System.Console.WriteLine(numberoffailedNIDS + " remaining unmapped NIDS");
			}
		}
...
//ORIGINAL LINE: private void LoadELFImports(pspsharp.HLE.kernel.types.SceModule module) throws java.io.IOException
		private void LoadELFImports(SceModule module)
		{
			...
			for (int i = 0; stubHeadersAddress < stubHeadersEndAddress; i++)
			{
				...
				{
					System.Console.WriteLine("Skipping dummy entry with size " + stubHeader.Size);
				...
				else
				{
					...
					//if (log.DebugEnabled)
					{
						System.Console.WriteLine(string.Format("Processing Import #{0:D}: {1}", i, stubHeader.ToString()));
					}
					...
					if (stubHeader.hasVStub())
					{
						//if (log.DebugEnabled)
						{
							System.Console.WriteLine(string.Format("'{0}': Header with VStub has size {1:D}: {2}", stubHeader.ModuleNamez, stubHeader.Size, Utilities.getMemoryDump(stubHeadersAddress, stubHeader.Size * 4, 4, 16)));
						}
						...
						if (vStub != 0)
						{
							...
							//if (log.DebugEnabled)
							{
								System.Console.WriteLine(string.Format("VStub has size {0:D}: {1}", vStubSize, Utilities.getMemoryDump(vStub, vStubSize * 8, 4, 16)));
							}
								...
								while (true)
								{
									...
									switch (opcode)
									{
										...
										case AllegrexOpcodes.J:
											...
											break;
										default:
											System.Console.WriteLine(string.Format("Unknown Vstub relocation nid 0x{0:X8}, reloc=0x{1:X8}", nid, reloc));
											break;
									}
									...
									if (deferredStub != null)
									{
										//if (log.DebugEnabled)
										{
											System.Console.WriteLine(string.Format("Vstub reloc {0}", deferredStub));
										}
					...
					{
						System.Console.WriteLine(string.Format("Incorrect s_nid or s_text address in StubHeader #{0:D}: {1}", i, stubHeader.ToString()));
					}
					else
					{
			...
			if (module.unresolvedImports.Count > 0)
			{
				if (log.InfoEnabled)
				{
					System.Console.WriteLine(string.Format("Found {0:D} unresolved imports", module.unresolvedImports.Count));
				}
			}
		}
...
//ORIGINAL LINE: private void LoadELFExports(pspsharp.HLE.kernel.types.SceModule module) throws java.io.IOException
		private void LoadELFExports(SceModule module)
		{
			...
			for (int i = 0; entHeadersAddress < entHeadersEndAddress; i++)
			{
				...
				{
					// Skip 0 sized entries.
					System.Console.WriteLine("Skipping dummy entry with size " + entHeader.Size);
				...
				else
				{
					...
					//if (log.DebugEnabled)
					{
						System.Console.WriteLine(string.Format("Processing header #{0:D} at 0x{1:X8}: {2}", i, entHeadersAddress, entHeader.ToString()));
					}
					...
					if (entHeader.Size > 4)
					{
						//if (log.DebugEnabled)
						{
							System.Console.WriteLine(string.Format("'{0}': Header has size {1:D}: {2}", entHeader.ModuleNamez, entHeader.Size, Utilities.getMemoryDump(entHeadersAddress, entHeader.Size * 4, 4, 16)));
						}
					...
					if ((entHeader.Attr & 0x8000) == 0)
					{
						for (int j = 0; j < functionCount; j++)
						{
							...
							if (Memory.isAddressGood(exportAddress) && ((entHeader.Attr & 0x4000) != 0x4000) || module.pspfilename.StartsWith("ms0:", StringComparison.Ordinal) || module.pspfilename.StartsWith("disc0:/PSP_GAME/SYSDIR/EBOOT.", StringComparison.Ordinal) || module.pspfilename.StartsWith("flash0:", StringComparison.Ordinal))
							{
								...
								//if (log.DebugEnabled)
								{
									System.Console.WriteLine(string.Format("Export found at 0x{0:X8} [0x{1:X8}]", exportAddress, nid));
								}
							}
						}
					}
						...
						for (int j = 0; j < functionCount; j++)
						{
							...
							switch (nid)
							{
								case 0xD632ACDB: // module_start
									...
									//if (log.DebugEnabled)
									{
										System.Console.WriteLine(string.Format("module_start found: nid=0x{0:X8}, function=0x{1:X8}", nid, exportAddress));
									}
									break;
								case 0xCEE8593C: // module_stop
									...
									//if (log.DebugEnabled)
									{
										System.Console.WriteLine(string.Format("module_stop found: nid=0x{0:X8}, function=0x{1:X8}", nid, exportAddress));
									}
									break;
								case 0x2F064FA6: // module_reboot_before
									...
									//if (log.DebugEnabled)
									{
										System.Console.WriteLine(string.Format("module_reboot_before found: nid=0x{0:X8}, function=0x{1:X8}", nid, exportAddress));
									}
									break;
								case 0xADF12745: // module_reboot_phase
									...
									//if (log.DebugEnabled)
									{
										System.Console.WriteLine(string.Format("module_reboot_phase found: nid=0x{0:X8}, function=0x{1:X8}", nid, exportAddress));
									}
									break;
								case 0xD3744BE0: // module_bootstart
									...
									//if (log.DebugEnabled)
									{
										System.Console.WriteLine(string.Format("module_bootstart found: nid=0x{0:X8}, function=0x{1:X8}", nid, exportAddress));
									}
									break;
								default:
					...
					for (int j = 0; j < variableCount; j++)
					{
						...
						switch (nid)
						{
							case 0xF01D73A7: // module_info
								...
								//if (log.DebugEnabled)
								{
									System.Console.WriteLine(string.Format("module_info found: nid=0x{0:X8}, addr=0x{1:X8}", nid, variableAddr));
								}
								break;
							case 0x0F7C276C: // module_start_thread_parameter
								...
								//if (log.DebugEnabled)
								{
									System.Console.WriteLine(string.Format("module_start_thread_parameter found: nid=0x{0:X8}, priority={1:D}, stacksize={2:D}, attr=0x{3:X8}", nid, module.module_start_thread_priority, module.module_start_thread_stacksize, module.module_start_thread_attr));
								}
								break;
							case 0xCF0CC697: // module_stop_thread_parameter
								...
								//if (log.DebugEnabled)
								{
									System.Console.WriteLine(string.Format("module_stop_thread_parameter found: nid=0x{0:X8}, priority={1:D}, stacksize={2:D}, attr=0x{3:X8}", nid, module.module_stop_thread_priority, module.module_stop_thread_stacksize, module.module_stop_thread_attr));
								}
								break;
							case 0xF4F4299D: // module_reboot_before_thread_parameter
								...
								//if (log.DebugEnabled)
								{
									System.Console.WriteLine(string.Format("module_reboot_before_thread_parameter found: nid=0x{0:X8}, priority={1:D}, stacksize={2:D}, attr=0x{3:X8}", nid, module.module_reboot_before_thread_priority, module.module_reboot_before_thread_stacksize, module.module_reboot_before_thread_attr));
								}
								break;
							case 0x11B97506: // module_sdk_version
								...
								//if (log.DebugEnabled)
								{
									System.Console.WriteLine(string.Format("module_sdk_version found: nid=0x{0:X8}, sdk_version=0x{1:X8}", nid, sdk_version));
								}
								break;
							default:
								// Only accept exports from custom modules (attr != 0x4000) and with valid export addresses.
								if (Memory.isAddressGood(variableAddr) && ((entHeader.Attr & 0x4000) != 0x4000))
								{
									...
									//if (log.DebugEnabled)
									{
										System.Console.WriteLine(string.Format("Export found at 0x{0:X8} [0x{1:X8}]", variableAddr, nid));
									}
								}
								else
								{
									System.Console.WriteLine(string.Format("Unknown variable entry found: nid=0x{0:X8}, addr=0x{1:X8}", nid, variableAddr));
								}
								break;
						}
					}
			...
			if (entCount > 0)
			{
				if (log.InfoEnabled)
				{
					System.Console.WriteLine(string.Format("Found {0:D} exports", entCount));
				}
			}
		}

//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: private void LoadELFDebuggerInfo(ByteBuffer f, pspsharp.HLE.kernel.types.SceModule module, int baseAddress, pspsharp.format.Elf32 elf, int elfOffset, bool fromSyscall) throws java.io.IOException
