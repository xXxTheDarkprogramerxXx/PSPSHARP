...
//ORIGINAL LINE: return String.format("pesHeader=%s, frameEnd=0x%X, gotFrame=%b, image %dx%d", pesHeader, frameEnd, gotFrame, imageWidth, imageHeight);
		...
		public class PSMFStream
		{
			...
			public virtual void readMPEGVideoStreamParams(Memory mem, int addr, sbyte[] mpegHeader, int offset, PSMFHeader psmfHeader)
			{
				...
				if (log.InfoEnabled)
				{
					System.Console.WriteLine(string.Format("Found PSMF MPEG video stream data: streamID=0x{0:X}, privateStreamID=0x{1:X}, unk1=0x{2:X}, unk2=0x{3:X}, EPMapOffset=0x{4:x}, EPMapNumEntries={5:D}, frameWidth={6:D}, frameHeight={7:D}", streamID, privateStreamID, unk1, unk2, EPMapOffset, EPMapNumEntries, frameWidth, frameHeight));
				}
			...
			public virtual void readPrivateAudioStreamParams(Memory mem, int addr, sbyte[] mpegHeader, int offset, PSMFHeader psmfHeader)
			{
				...
				if (log.InfoEnabled)
				{
					System.Console.WriteLine(string.Format("Found PSMF MPEG audio stream data: streamID=0x{0:X}, privateStreamID=0x{1:X}, unk1=0x{2:X}, unk2=0x{3:X}, audioChannelConfig={4:D}, audioSampleFrequency={5:D}", streamID, privateStreamID, unk1, unk2, audioChannelConfig, audioSampleFrequency));
				}
			...
			{
				System.Console.WriteLine(string.Format("Unknown User Data stream format"));
					...
					foreach (PSMFStream stream in psmfStreams)
					{
						...
						if (stream.EPMapOffset + stream.EPMapNumEntries * 10 <= MPEG_HEADER_BUFFER_MINIMUM_SIZE)
						{
							...
							for (int i = 0; i < stream.EPMapNumEntries; i++)
							{
								...
								//if (log.DebugEnabled)
								{
									System.Console.WriteLine(string.Format("EPMap stream {0:D}, entry#{1:D}: {2}", stream.StreamChannel, i, psmfEntry));
								}
							}
						}
					}
		...
		public static LinkedList<PSMFStream> readPsmfStreams(Memory mem, int addr, sbyte[] mpegHeader, PSMFHeader psmfHeader)
		{
			...
			// Parse the stream FieldInfo and assign each one to it's type.
...
//JAVA TO C# CONVERTER NOTE: Fields cannot have the same name as methods:
			...
			public override void run()
			{
				...
				//if (log.DebugEnabled)
				{
					System.Console.WriteLine("Exiting the VideoDecoderThread");
				}
				done = true;
			}
...
//ORIGINAL LINE: log.trace(String.format("decodeImage returning avcGotFrame(0x%08X)=%b", gotFrameAddr.getAddress(), avcGotFrame));
		...
		private void removeErrorImages()
		{
			lock (decodedImages)
			{
				while (DecoderInErrorCondition)
				{
					//if (log.DebugEnabled)
					{
						System.Console.WriteLine(string.Format("Removing error image {0}", decodedImages.First.Value));
					}
		...
		private void decodeNextImage(TPointer auAddr)
		{
			...
			if (decodedImageInfo.frameEnd >= 0)
			{
				...
				lock (videoBuffer)
				{
					...
					if (result < 0)
					{
						System.Console.WriteLine(string.Format("decodeNextImage codec returned 0x{0:X8}", result));
			...
			//if (log.DebugEnabled)
			{
				System.Console.WriteLine(string.Format("Adding decoded image {0}", decodedImageInfo));
			}
			lock (decodedImages)
			{
		...
		private void hleVideoDecoderStep(int threadUid, int buffer, int frameWidth, int pixelMode, TPointer32 gotFrameAddr, bool writeAbgr, TPointer auAddr, long threadWakeupMicroTime)
		{
			//if (log.DebugEnabled)
			{
				...
				{
//JAVA TO C# CONVERTER TODO TASK: The following line has a Java format specifier which cannot be directly translated to .NET:
//ORIGINAL LINE: System.Console.WriteLine(String.format("hleVideoDecoderStep threadUid=0x%X, buffer=0x%08X, frameWidth=%d, pixelMode=%d, gotFrameAddr=%s, writeAbgr=%b, %d decoded images", threadUid, buffer, frameWidth, pixelMode, gotFrameAddr, writeAbgr, decodedImages.size()));
					System.Console.WriteLine(string.Format("hleVideoDecoderStep threadUid=0x%X, buffer=0x%08X, frameWidth=%d, pixelMode=%d, gotFrameAddr=%s, writeAbgr=%b, %d decoded images", threadUid, buffer, frameWidth, pixelMode, gotFrameAddr, writeAbgr, decodedImages.Count));
				}
				else
				{
					System.Console.WriteLine(string.Format("hleVideoDecoderStep {0:D} decoded images", decodedImages.Count));
				}
			}
			...
			while (Length > 0)
			{
				...
				if (currentFrameLength == 0)
				{
					...
					if (frameHeader[0] != 0x0F || frameHeader[1] != 0xD0)
					{
						if (log.InfoEnabled)
						{
							System.Console.WriteLine(string.Format("Audio frame Length 0x{0:X} with incorrect header (header: {1:X2} {2:X2} {3:X2} {4:X2} {5:X2} {6:X2} {7:X2} {8:X2})", audioFrameLength, frameHeader[0], frameHeader[1], frameHeader[2], frameHeader[3], frameHeader[4], frameHeader[5], frameHeader[6], frameHeader[7]));
						}
					}
					else
					{
				...
				{
					System.Console.WriteLine(string.Format("AudioBuffer too small"));
				}
		...
		private void readNextAudioFrame(PesHeader pesHeader)
		{
			...
			//if (log.DebugEnabled)
			{
				System.Console.WriteLine(string.Format("readNextAudioFrame {0}", mpegRingbuffer));
			}
		...
		case 0x1EF:
					...
					default:
						...
						//if (log.DebugEnabled)
						{
							System.Console.WriteLine(string.Format("Unknown StartCode 0x{0:X8} at 0x{1:X8}", startCode, buffer.ReadAddr - 4));
						}
						break;
				}
			}
			...
			//if (log.DebugEnabled)
			{
				System.Console.WriteLine(string.Format("After readNextAudioFrame {0}", mpegRingbuffer));
			}
		}
		...
		private int readNextVideoFrame(PesHeader pesHeader, TPointer auAddr)
		{
			...
			//if (log.DebugEnabled)
			{
				System.Console.WriteLine(string.Format("readNextVideoFrame {0}", mpegRingbuffer));
			}
		...
		case 0x1EF:
			...
			//if (log.DebugEnabled)
			{
				System.Console.WriteLine(string.Format("After readNextVideoFrame frameEnd=0x{0:X}, {1}", frameEnd, mpegRingbuffer));
			}
		...
		private void readNextUserDataFrame(PesHeader pesHeader)
		{
			...
			//if (log.DebugEnabled)
			{
				System.Console.WriteLine(string.Format("readNextUserDataFrame {0}", mpegRingbuffer));
			}
		...
		case 0x1EF:
			...
			//if (log.DebugEnabled)
			{
				System.Console.WriteLine(string.Format("After readNextUserDataFrame {0}", mpegRingbuffer));
			}
		}
		...
		private void writeImageABGR(int addr, int frameWidth, int imageWidth, int imageHeight, int pixelMode, int[] abgr)
		{
			...
			//if (log.DebugEnabled)
			{
				System.Console.WriteLine(string.Format("writeImageABGR addr=0x{0:X8}-0x{1:X8}, frameWidth={2:D}, frameHeight={3:D}, width={4:D}, height={5:D}, pixelMode={6:D}", addr, addr + frameWidth * frameHeight * bytesPerPixel, frameWidth, frameHeight, imageWidth, imageHeight, pixelMode));
			}
		...
		private void writeImageYCbCr(int addr, int imageWidth, int imageHeight, int[] luma, int[] cb, int[] cr)
		{
			...
			//if (log.DebugEnabled)
			{
				System.Console.WriteLine(string.Format("writeImageYCbCr addr=0x{0:X8}-0x{1:X8}, frameWidth={2:D}, frameHeight={3:D}", addr, addr + Length + length2 + length2, frameWidth, frameHeight));
			}
		...
		private bool getImage(DecodedImageInfo decodedImageInfo)
		{
			...
			if (result < 0)
			{
				System.Console.WriteLine(string.Format("VideoCodec error 0x{0:X8} while retrieving the image", result));
				return false;
			}
...
//ORIGINAL LINE: public int hleMpegCreate(pspsharp.HLE.TPointer mpeg, pspsharp.HLE.TPointer data, int size, @CanBeNull pspsharp.HLE.TPointer ringbufferAddr, int frameWidth, int mode, int ddrtop)
		public virtual int hleMpegCreate(TPointer mpeg, TPointer data, int size, TPointer ringbufferAddr, int frameWidth, int mode, int ddrtop)
		{
			...
			{
				System.Console.WriteLine("sceMpegCreate bad size " + size);
		...
		protected internal virtual void hleMpegRingbufferPostPut(AfterRingbufferPutCallback afterRingbufferPutCallback, int packetsAdded)
		{
			...
			if (packetsAdded > 0)
			{
				...
				{
					System.Console.WriteLine(string.Format("sceMpegRingbufferPut clamping packetsAdded old={0:D}, new={1:D}", packetsAdded, mpegRingbuffer.FreePackets));
				...
				//if (log.DebugEnabled)
				{
					System.Console.WriteLine(string.Format("sceMpegRingbufferPut packetsAdded=0x{0:X}, packetsRead=0x{1:X}, new availableSize=0x{2:X}", packetsAdded, mpegRingbuffer.ReadPackets, mpegRingbuffer.FreePackets));
				}
				...
				if (psmfHeader != null && dataSizeInRingbuffer > psmfHeader.mpegStreamSize)
				{
					//if (log.DebugEnabled)
					{
						System.Console.WriteLine(string.Format("sceMpegRingbufferPut returning ERROR_MPEG_INVALID_VALUE, size of data in ringbuffer=0x{0:X}, mpegStreamSize=0x{1:X}", dataSizeInRingbuffer, psmfHeader.mpegStreamSize));
					}
				...
				if (remainingPackets > 0)
				{
					...
					//if (log.DebugEnabled)
					{
						System.Console.WriteLine(string.Format("sceMpegRingbufferPut executing callback 0x{0:X8} to read 0x{1:X} packets at 0x{2:X8}", mpegRingbuffer.CallbackAddr, putNumberPackets, putDataAddr));
					}
			...
			else
			{
				...
				//if (log.DebugEnabled)
				{
					System.Console.WriteLine(string.Format("sceMpegRingbufferPut callback returning packetsAdded=0x{0:X}", packetsAdded));
				}
			}
		}
		...
		public virtual int hleMpegGetAvcAu(TPointer auAddr)
		{
			...
			if (RegisteredVideoChannel)
			{
				...
				while (true)
				{
					...
					//if (log.DebugEnabled)
					{
						System.Console.WriteLine(string.Format("hleMpegGetAvcAu waiting for the video decoder thread..."));
					}
					Utilities.sleep(1);
				}
				...
				//if (log.DebugEnabled)
				{
					System.Console.WriteLine(string.Format("hleMpegGetAvcAu decodedImageInfo: {0}", decodedImageInfo));
				}
				...
				if (decodedImageInfo.frameEnd < 0)
				{
					...
					if (psmfHeader == null || currentVideoTimestamp > psmfHeader.mpegLastTimestamp || !RegisteredAudioChannel)
					{
						//if (log.DebugEnabled)
						{
							if (psmfHeader == null)
							{
								System.Console.WriteLine(string.Format("hleMpegGetAvcAu with psmfHeader==null, returning ERROR_MPEG_NO_DATA"));
							}
							else
							{
								System.Console.WriteLine(string.Format("hleMpegGetAvcAu with currentVideoTimestamp={0:D} and psmfHeader.mpegLastTimestamp={1:D}, returning ERROR_MPEG_NO_DATA", currentVideoTimestamp, psmfHeader.mpegLastTimestamp));
							}
						}
			...
			//if (log.DebugEnabled)
			{
				System.Console.WriteLine(string.Format("hleMpegGetAvcAu returning 0x{0:X8}, AvcAu={1}", result, mpegAvcAu));
			}
		...
		public virtual int hleMpegGetAtracAu(TPointer auAddr)
		{
			...
			//if (log.DebugEnabled)
			{
				System.Console.WriteLine(string.Format("hleMpegGetAtracAu returning result=0x{0:X8}, {1}", result, mpegAtracAu));
			}
		...
		public virtual int hleMpegAtracDecode(TPointer auAddr, TPointer bufferAddr, int bufferSize)
		{
			...
			if (audioBuffer != null && audioFrameLength > 0 && audioBuffer.Length >= audioFrameLength)
			{
				...
				if (result < 0)
				{
					System.Console.WriteLine(string.Format("Error received from codec.decode: 0x{0:X8}", result));
				}
				else
				{
					if (log.TraceEnabled)
				...
				{
					System.Console.WriteLine(string.Format("Internal error while consuming from the audio buffer"));
				}
		...
		public virtual int checkMpegHandle(int mpeg)
		{
			...
			{
				System.Console.WriteLine(string.Format("checkMpegHandler bad mpeg handle 0x{0:X8}", mpeg));
		...
		protected internal virtual void analyseMpeg(int bufferAddr)
		{
			...
			//if (log.DebugEnabled)
			{
				System.Console.WriteLine(string.Format("Stream offset: 0x{0:X}, Stream size: 0x{1:X}", psmfHeader.mpegOffset, psmfHeader.mpegStreamSize));
				System.Console.WriteLine(string.Format("First timestamp: {0:D}, Last timestamp: {1:D}", psmfHeader.mpegFirstTimestamp, psmfHeader.mpegLastTimestamp));
				if (log.TraceEnabled)
				{
...
//ORIGINAL LINE: sealed override int bytesPerPixel = sceDisplay.getPixelFormatBytes(pixelMode);
			int bytesPerPixel = sceDisplay.getPixelFormatBytes(pixelMode);
			for (int y = 0; y < imageHeight - pixelSize + 1; y += pixelSize)
			{
				int address = dest_addr + y * frameWidth * bytesPerPixel;
//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'sealed override':
//ORIGINAL LINE: sealed override int width = Math.min(imageWidth, frameWidth);
		...
		protected internal virtual void finishStreams()
		{
			//if (log.DebugEnabled)
			{
				System.Console.WriteLine("finishStreams");
			}
		...
		protected internal virtual void finishMpeg()
		{
			//if (log.DebugEnabled)
			{
				System.Console.WriteLine("finishMpeg");
			}
		...
		protected internal virtual void checkEmptyVideoRingbuffer()
		{
			...
			if (mpegRingbuffer == null)
			{
				System.Console.WriteLine("ringbuffer not created");
			...
			{
				delayThread(mpegDecodeErrorDelay);
				System.Console.WriteLine("ringbuffer and video buffer are empty");
				throw new SceKernelErrorException(SceKernelErrors.ERROR_MPEG_NO_DATA); // No more data in ringbuffer.
			}
		}
		...
		protected internal virtual void checkEmptyAudioRingbuffer()
		{
			...
			{
				System.Console.WriteLine("ringbuffer and audio buffer are empty");
				delayThread(mpegDecodeErrorDelay);
				throw new SceKernelErrorException(SceKernelErrors.ERROR_MPEG_NO_DATA); // No more data in ringbuffer.
			}
		}
...
//ORIGINAL LINE: @HLEFunction(nid = 0x21FF80E4, version = 150, checkInsideInterrupt = true, stackUsage = 0x18) public int sceMpegQueryStreamOffset(@CheckArgument("checkMpegHandle") int mpeg, pspsharp.HLE.TPointer bufferAddr, @BufferInfo(usage=pspsharp.HLE.BufferInfo.Usage.out) pspsharp.HLE.TPointer32 offsetAddr)
		...
		public virtual int sceMpegQueryStreamOffset(int mpeg, TPointer bufferAddr, TPointer32 offsetAddr)
		{
			...
			{
				System.Console.WriteLine("sceMpegQueryStreamOffset bad magic " + string.Format("0x{0:X8}", psmfHeader.mpegMagic));
			...
			{
				System.Console.WriteLine("sceMpegQueryStreamOffset bad version " + string.Format("0x{0:X8}", psmfHeader.mpegRawVersion));
				offsetAddr.setValue(0);
			...
			{
				System.Console.WriteLine("sceMpegQueryStreamOffset bad offset " + string.Format("0x{0:X8}", psmfHeader.mpegOffset));
				offsetAddr.setValue(0);
				return SceKernelErrors.ERROR_MPEG_INVALID_VALUE;
			}
			...
			//if (log.DebugEnabled)
			{
				System.Console.WriteLine(string.Format("sceMpegQueryStreamOffset returning 0x{0:X}", offsetAddr.getValue()));
			}
...
//ORIGINAL LINE: @HLEFunction(nid = 0x611E9E11, version = 150, checkInsideInterrupt = true, stackUsage = 0x8) public int sceMpegQueryStreamSize(pspsharp.HLE.TPointer bufferAddr, @BufferInfo(usage=pspsharp.HLE.BufferInfo.Usage.out) pspsharp.HLE.TPointer32 sizeAddr)
		...
		public virtual int sceMpegQueryStreamSize(TPointer bufferAddr, TPointer32 sizeAddr)
		{
			...
			if (psmfHeader.mpegMagic != PSMF_MAGIC)
			{
				System.Console.WriteLine(string.Format("sceMpegQueryStreamSize bad magic 0x{0:X8}", psmfHeader.mpegMagic));
				return -1;
			}
...
//ORIGINAL LINE: @HLEFunction(nid = 0x42560F23, version = 150, checkInsideInterrupt = true, stackUsage = 0x48) public int sceMpegRegistStream(@CheckArgument("checkMpegHandle") int mpeg, int streamType, int streamChannelNum)
		...
		public virtual int sceMpegRegistStream(int mpeg, int streamType, int streamChannelNum)
		{
			...
			//if (log.DebugEnabled)
			{
				System.Console.WriteLine(string.Format("sceMpegRegistStream returning 0x{0:X}", info.Uid));
			}
...
//ORIGINAL LINE: @HLEFunction(nid = 0x591A4AA2, version = 150, checkInsideInterrupt = true, stackUsage = 0x18) public int sceMpegUnRegistStream(@CheckArgument("checkMpegHandle") int mpeg, int streamUid)
		...
		public virtual int sceMpegUnRegistStream(int mpeg, int streamUid)
		{
			...
			if (info == null)
			{
				System.Console.WriteLine(string.Format("sceMpegUnRegistStream unknown stream=0x{0:X}", streamUid));
...
//ORIGINAL LINE: @HLEFunction(nid = 0xCEB870B1, version = 150, checkInsideInterrupt = true, stackUsage = 0x28) public int sceMpegFreeAvcEsBuf(@CheckArgument("checkMpegHandle") int mpeg, int esBuf)
		...
		public virtual int sceMpegFreeAvcEsBuf(int mpeg, int esBuf)
		{
			if (esBuf == 0)
			{
				System.Console.WriteLine("sceMpegFreeAvcEsBuf(mpeg=0x" + mpeg.ToString("x") + ", esBuf=0x" + esBuf.ToString("x") + ") bad esBuf handle");
				return SceKernelErrors.ERROR_MPEG_INVALID_VALUE;
			}
...
//ORIGINAL LINE: @HLEUnimplemented @HLEFunction(nid = 0x234586AE, version = 150, checkInsideInterrupt = true) public int sceMpegChangeGetAvcAuMode(int mpeg, int stream_addr, int mode)
		...
		public virtual int sceMpegChangeGetAuMode(int mpeg, int streamUid, int mode)
		{
			...
			if (info == null)
			{
				System.Console.WriteLine(string.Format("sceMpegChangeGetAuMode unknown stream=0x{0:X}", streamUid));
				return -1;
			}
			...
			//if (log.DebugEnabled)
			{
				System.Console.WriteLine(string.Format("sceMpegChangeGetAuMode mode={0}: {1}", mode == MPEG_AU_MODE_DECODE ? "DECODE" : "SKIP", info));
			}
...
//ORIGINAL LINE: @HLEFunction(nid = 0xFE246728, version = 150, checkInsideInterrupt = true) public int sceMpegGetAvcAu(@CheckArgument("checkMpegHandle") int mpeg, int streamUid, @BufferInfo(lengthInfo=pspsharp.HLE.BufferInfo.LengthInfo.fixedLength, Length=24, usage=pspsharp.HLE.BufferInfo.Usage.out) pspsharp.HLE.TPointer auAddr, @CanBeNull pspsharp.HLE.TPointer32 attrAddr)
		...
		public virtual int sceMpegGetAvcAu(int mpeg, int streamUid, TPointer auAddr, TPointer32 attrAddr)
		{
			...
			{
				System.Console.WriteLine("sceMpegGetAvcAu didn't get a fake stream");
				return SceKernelErrors.ERROR_MPEG_INVALID_ADDR;
			}

			if (!streamMap.ContainsKey(streamUid))
			{
				System.Console.WriteLine(string.Format("sceMpegGetAvcAu bad stream 0x{0:X}", streamUid));
				return -1;
			}
			...
			//if (log.DebugEnabled)
			{
				System.Console.WriteLine(string.Format("videoFrameCount={0:D}(pts={1:D}), audioFrameCount={2:D}(pts={3:D}), pts difference {4:D}, vcount={5:D}", videoFrameCount, currentVideoTimestamp, audioFrameCount, currentAudioTimestamp, currentAudioTimestamp - currentVideoTimestamp, Modules.sceDisplayModule.Vcount));
			}
...
//ORIGINAL LINE: @HLEFunction(nid = 0x8C1E027D, version = 150, checkInsideInterrupt = true) public int sceMpegGetPcmAu(@CheckArgument("checkMpegHandle") int mpeg, int streamUid, @BufferInfo(lengthInfo=pspsharp.HLE.BufferInfo.LengthInfo.fixedLength, Length=24, usage=pspsharp.HLE.BufferInfo.Usage.out) pspsharp.HLE.TPointer auAddr, @CanBeNull pspsharp.HLE.TPointer32 attrAddr)
		...
		public virtual int sceMpegGetPcmAu(int mpeg, int streamUid, TPointer auAddr, TPointer32 attrAddr)
		{
			...
			if (Memory.isAddressGood(streamUid))
			{
				System.Console.WriteLine("sceMpegGetPcmAu didn't get a fake stream");
				return SceKernelErrors.ERROR_MPEG_INVALID_ADDR;
			}

			if (!streamMap.ContainsKey(streamUid))
			{
				System.Console.WriteLine(string.Format("sceMpegGetPcmAu bad streamUid 0x{0:X8}", streamUid));
				return -1;
			}
			int result = 0;
			...
			if (RegisteredPcmChannel >= 0)
			{
				...
				//if (log.DebugEnabled)
				{
					System.Console.WriteLine(string.Format("sceMpegGetPcmAu returning AtracAu={0}", mpegAtracAu.ToString()));
				}
			}
...
//ORIGINAL LINE: @HLEFunction(nid = 0xE1CE83A7, version = 150, checkInsideInterrupt = true) public int sceMpegGetAtracAu(@CheckArgument("checkMpegHandle") int mpeg, int streamUid, @BufferInfo(lengthInfo=pspsharp.HLE.BufferInfo.LengthInfo.fixedLength, Length=24, usage=pspsharp.HLE.BufferInfo.Usage.out) pspsharp.HLE.TPointer auAddr, @CanBeNull pspsharp.HLE.TPointer32 attrAddr)
		...
		public virtual int sceMpegGetAtracAu(int mpeg, int streamUid, TPointer auAddr, TPointer32 attrAddr)
		{
			...
			if (Memory.isAddressGood(streamUid))
			{
				System.Console.WriteLine("sceMpegGetAtracAu didn't get a fake stream");
				return SceKernelErrors.ERROR_MPEG_INVALID_ADDR;
			}

			if (!streamMap.ContainsKey(streamUid))
			{
				System.Console.WriteLine("sceMpegGetAtracAu bad address " + string.Format("0x{0:X8} 0x{1:X8}", streamUid, auAddr));
				return -1;
			}
...
//ORIGINAL LINE: @HLEFunction(nid = 0x0E3C2E9D, version = 150, checkInsideInterrupt = true) public int sceMpegAvcDecode(@CheckArgument("checkMpegHandle") int mpeg, @BufferInfo(lengthInfo=pspsharp.HLE.BufferInfo.LengthInfo.fixedLength, Length=24, usage=pspsharp.HLE.BufferInfo.Usage.inout) pspsharp.HLE.TPointer auAddr, int frameWidth, @CanBeNull pspsharp.HLE.TPointer32 bufferAddr, @BufferInfo(usage=pspsharp.HLE.BufferInfo.Usage.out) pspsharp.HLE.TPointer32 gotFrameAddr)
		...
		public virtual int sceMpegAvcDecode(int mpeg, TPointer auAddr, int frameWidth, TPointer32 bufferAddr, TPointer32 gotFrameAddr)
		{
			...
			{
//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'sealed override':
//ORIGINAL LINE: sealed override int width = frameWidth;
				int width = frameWidth;
//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'sealed override':
//ORIGINAL LINE: sealed override int height = width < 480 ? 160 : 272;
				...
				//if (log.DebugEnabled)
				{
					System.Console.WriteLine(string.Format("sceMpegAvcDecode buffer=0x{0:X8}, avcEsBuf: {1}", buffer, Utilities.getMemoryDump(avcEsBuf.addr, AVC_ES_BUF_SIZE)));
				}
...
//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'sealed override':
//ORIGINAL LINE: sealed override int height = psmfHeader != null ? psmfHeader.getVideoHeight() : 272;
			...
			{
//JAVA TO C# CONVERTER TODO TASK: The following line has a Java format specifier which cannot be directly translated to .NET:
//ORIGINAL LINE: System.Console.WriteLine(String.format("sceMpegAvcDecode buffer=0x%08X, dts=0x%X, pts=0x%X, gotFrame=%b", buffer, mpegAvcAu.dts, mpegAvcAu.pts, avcGotFrame));
				System.Console.WriteLine(string.Format("sceMpegAvcDecode buffer=0x%08X, dts=0x%X, pts=0x%X, gotFrame=%b", buffer, mpegAvcAu.dts, mpegAvcAu.pts, avcGotFrame));
			}
...
//ORIGINAL LINE: @HLEFunction(nid = 0x0F6C18D7, version = 150, checkInsideInterrupt = true) public int sceMpegAvcDecodeDetail(@CheckArgument("checkMpegHandle") int mpeg, pspsharp.HLE.TPointer detailPointer)
		...
		public virtual int sceMpegAvcDecodeDetail(int mpeg, TPointer detailPointer)
		{
			...
			//if (log.DebugEnabled)
			{
				System.Console.WriteLine(string.Format("sceMpegAvcDecodeDetail returning decodeResult=0x{0:X}, frameCount={1:D}, width={2:D}, height={3:D}, gotFrame=0x{4:X}", detailPointer.getValue32(0), detailPointer.getValue32(4), detailPointer.getValue32(8), detailPointer.getValue32(12), detailPointer.getValue32(32)));
			}
			return 0;
		}
...
//ORIGINAL LINE: @HLEFunction(nid = 0xA11C7026, version = 150, checkInsideInterrupt = true) public int sceMpegAvcDecodeMode(@CheckArgument("checkMpegHandle") int mpeg, @BufferInfo(lengthInfo=pspsharp.HLE.BufferInfo.LengthInfo.fixedLength, Length=8, usage=pspsharp.HLE.BufferInfo.Usage.in) pspsharp.HLE.TPointer32 modeAddr)
		...
		public virtual int sceMpegAvcDecodeMode(int mpeg, TPointer32 modeAddr)
		{
			...
			if (pixelMode >= TPSM_PIXEL_STORAGE_MODE_16BIT_BGR5650 && pixelMode <= TPSM_PIXEL_STORAGE_MODE_32BIT_ABGR8888)
			{
				//if (log.DebugEnabled)
				{
					System.Console.WriteLine(string.Format("sceMpegAvcDecodeMode mode=0x{0:X}, pixelMode=0x{1:X}", mode, pixelMode));
				}
				videoPixelMode = pixelMode;
			}
			else
			{
				System.Console.WriteLine(string.Format("sceMpegAvcDecodeMode mode=0x{0:X}, pixel mode=0x{1:X}: unknown pixel mode", mode, pixelMode));
			}
			return 0;
		}
...
//ORIGINAL LINE: @HLEFunction(nid = 0x211A057C, version = 150, checkInsideInterrupt = true) public int sceMpegAvcQueryYCbCrSize(@CheckArgument("checkMpegHandle") int mpeg, int mode, int width, int height, pspsharp.HLE.TPointer32 resultAddr)
		...
		public virtual int sceMpegAvcQueryYCbCrSize(int mpeg, int mode, int width, int height, TPointer32 resultAddr)
		{
			...
			{
				System.Console.WriteLine("sceMpegAvcQueryYCbCrSize invalid size width=" + width + ", height=" + height);
				return SceKernelErrors.ERROR_MPEG_INVALID_VALUE;
			}
...
//ORIGINAL LINE: @HLEFunction(nid = 0xF0EB1125, version = 150, checkInsideInterrupt = true) public int sceMpegAvcDecodeYCbCr(@CheckArgument("checkMpegHandle") int mpeg, pspsharp.HLE.TPointer auAddr, pspsharp.HLE.TPointer32 bufferAddr, pspsharp.HLE.TPointer32 gotFrameAddr)
		...
		public virtual int sceMpegAvcDecodeYCbCr(int mpeg, TPointer auAddr, TPointer32 bufferAddr, TPointer32 gotFrameAddr)
		{
			...
			{
//JAVA TO C# CONVERTER TODO TASK: The following line has a Java format specifier which cannot be directly translated to .NET:
//ORIGINAL LINE: System.Console.WriteLine(String.format("sceMpegAvcDecodeYCbCr *buffer=0x%08X, currentTimestamp=%d, avcGotFrame=%b", bufferAddr.getValue(), mpegAvcAu.pts, avcGotFrame));
				System.Console.WriteLine(string.Format("sceMpegAvcDecodeYCbCr *buffer=0x%08X, currentTimestamp=%d, avcGotFrame=%b", bufferAddr.getValue(), mpegAvcAu.pts, avcGotFrame));
			}
...
//ORIGINAL LINE: @HLEFunction(nid = 0x31BD0272, version = 150, checkInsideInterrupt = true) public int sceMpegAvcCsc(@CheckArgument("checkMpegHandle") int mpeg, pspsharp.HLE.TPointer sourceAddr, pspsharp.HLE.TPointer32 rangeAddr, int frameWidth, pspsharp.HLE.TPointer destAddr)
		...
		public virtual int sceMpegAvcCsc(int mpeg, TPointer sourceAddr, TPointer32 rangeAddr, int frameWidth, TPointer destAddr)
		{
			...
			//if (log.DebugEnabled)
			{
				System.Console.WriteLine(string.Format("sceMpegAvcCsc range x={0:D}, y={1:D}, width={2:D}, height={3:D}", rangeX, rangeY, rangeWidth, rangeHeight));
			}
			...
			if (((rangeX | rangeY | rangeWidth | rangeHeight) & 0xF) != 0)
			{
				//if (log.DebugEnabled)
				{
					System.Console.WriteLine(string.Format("sceMpegAvcCsc returning ERROR_MPEG_INVALID_VALUE"));
				}
				return SceKernelErrors.ERROR_MPEG_INVALID_VALUE;
			}
			...
			if (rangeX < 0 || rangeY < 0 || rangeWidth < 0 || rangeHeight < 0)
			{
				...
				//if (log.DebugEnabled)
				{
					System.Console.WriteLine(string.Format("sceMpegAvcCsc returning ERROR_INVALID_VALUE"));
				}
...
//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'sealed override':
//ORIGINAL LINE: sealed override int bytesPerPixel = sceDisplay.getPixelFormatBytes(videoPixelMode);
			...
			//if (log.DebugEnabled)
			{
				System.Console.WriteLine(string.Format("sceMpegAvcCsc writing to 0x{0:X8}-0x{1:X8}, vcount={2:D}", destAddr.Address, destAddr.Address + (rangeY + rangeHeight) * frameWidth * bytesPerPixel, Modules.sceDisplayModule.Vcount));
			}
...
//ORIGINAL LINE: @HLEFunction(nid = 0x800C44DF, version = 150, checkInsideInterrupt = true) public int sceMpegAtracDecode(@CheckArgument("checkMpegHandle") int mpeg, pspsharp.HLE.TPointer auAddr, pspsharp.HLE.TPointer bufferAddr, int init)
		...
		public virtual int sceMpegAtracDecode(int mpeg, TPointer auAddr, TPointer bufferAddr, int init)
		{
			...
			//if (log.DebugEnabled)
			{
				System.Console.WriteLine(string.Format("sceMpegAtracDecode currentTimestamp={0:D}", mpegAtracAu.pts));
			}
...
//ORIGINAL LINE: @HLEFunction(nid = 0x37295ED8, version = 150, checkInsideInterrupt = true, stackUsage = 0x38) public int sceMpegRingbufferConstruct(pspsharp.HLE.TPointer ringbufferAddr, int packets, @CanBeNull pspsharp.HLE.TPointer data, int size, @CanBeNull pspsharp.HLE.TPointer callbackAddr, int callbackArgs)
		...
		public virtual int sceMpegRingbufferConstruct(TPointer ringbufferAddr, int packets, TPointer data, int size, TPointer callbackAddr, int callbackArgs)
		{
			...
			{
				System.Console.WriteLine(string.Format("sceMpegRingbufferConstruct insufficient space: size={0:D}, packets={1:D}", size, packets));
				return SceKernelErrors.ERROR_MPEG_NO_MEMORY;
			}
		...
		public virtual int sceMpegRingbufferPut(TPointer ringbufferAddr, int numPackets, int available)
		{
			...
			//if (log.DebugEnabled)
			{
				System.Console.WriteLine(string.Format("sceMpegRingbufferPut executing callback 0x{0:X8} to read 0x{1:X} packets at 0x{2:X8}, Ringbuffer={3}", mpegRingbuffer.CallbackAddr, putNumberPackets, putDataAddr, mpegRingbuffer));
			}
			Modules.ThreadManForUserModule.executeCallback(null, mpegRingbuffer.CallbackAddr, afterRingbufferPutCallback, false, putDataAddr, putNumberPackets, mpegRingbuffer.CallbackArgs);
...
//ORIGINAL LINE: @HLEUnimplemented @HLEFunction(nid = 0x3C37A7A6, version = 150, checkInsideInterrupt = true) public int sceMpegNextAvcRpAu(@CheckArgument("checkMpegHandle") int mpeg, int streamUid)
		...
		public virtual int sceMpegNextAvcRpAu(int mpeg, int streamUid)
		{
			if (!streamMap.ContainsKey(streamUid))
			{
				System.Console.WriteLine(string.Format("sceMpegNextAvcRpAu bad stream 0x{0:X}", streamUid));
				return -1;
			}
			...
			if (result != 0)
			{
				//if (log.DebugEnabled)
				{
					System.Console.WriteLine(string.Format("sceMpegNextAvcRpAu returning 0x{0:X8}", result));
				}
				return result;
			}
...
//ORIGINAL LINE: @HLEFunction(nid = 0x01977054, version = 150) public int sceMpegGetUserdataAu(@CheckArgument("checkMpegHandle") int mpeg, int streamUid, pspsharp.HLE.TPointer auAddr, @CanBeNull pspsharp.HLE.TPointer headerAddr)
		...
		public virtual int sceMpegGetUserdataAu(int mpeg, int streamUid, TPointer auAddr, TPointer headerAddr)
		{
			if (!hasPsmfUserdataStream())
			{
				//if (log.DebugEnabled)
				{
					System.Console.WriteLine(string.Format("sceMpegGetUserdataAu no registered user data stream, returning 0x{0:X8}", SceKernelErrors.ERROR_MPEG_NO_DATA));
				}
			...
			if (userDataLength == 0)
			{
				//if (log.DebugEnabled)
				{
					System.Console.WriteLine(string.Format("sceMpegGetUserdataAu no user data available, returning 0x{0:X8}", SceKernelErrors.ERROR_MPEG_NO_DATA));
				}
				return SceKernelErrors.ERROR_MPEG_NO_DATA;
			}
			if (userDataBuffer.Length < userDataLength)
			{
				//if (log.DebugEnabled)
				{
					System.Console.WriteLine(string.Format("sceMpegGetUserdataAu no enough user data available (0x{0:X} from 0x{1:X}), returning 0x{2:X8}", userDataBuffer.Length, userDataLength, SceKernelErrors.ERROR_MPEG_NO_DATA));
				}
				return SceKernelErrors.ERROR_MPEG_NO_DATA;
			}
			...
			//if (log.DebugEnabled)
			{
				System.Console.WriteLine(string.Format("sceMpegGetUserdataAu returning au={0}", mpegUserDataAu));
				if (log.TraceEnabled)
				{
...
//ORIGINAL LINE: @HLEFunction(nid = 0x0558B075, version = 150) public int sceMpegAvcCopyYCbCr(@CheckArgument("checkMpegHandle") int mpeg, pspsharp.HLE.TPointer destinationAddr, pspsharp.HLE.TPointer sourceAddr)
		...
		public virtual int sceMpegAvcCopyYCbCr(int mpeg, TPointer destinationAddr, TPointer sourceAddr)
		{
			...
			//if (log.DebugEnabled)
			{
				System.Console.WriteLine(string.Format("sceMpegAvcCopyYCbCr from 0x{0:X8}-0x{1:X8} to 0x{2:X8}-0x{3:X8}", sourceAddr.Address, sourceAddr.Address + size, destinationAddr.Address, destinationAddr.Address + size));
			}
		...
		public virtual int sceMpegGetAvcNalAu(int mpeg, TPointer mp4AvcNalStructAddr, TPointer auAddr)
		{
			...
			if (!hasVideoCodecExtraData())
			{
				...
				//if (log.DebugEnabled)
				{
					...
					System.Console.WriteLine(string.Format("sceMpegGetAvcNalAu videoCodecExtraData: {0}", Utilities.getMemoryDump(buffer, 0, buffer.Length)));
				}
			}
...
//ORIGINAL LINE: @HLEUnimplemented @HLEFunction(nid = 0xAB0E9556, version = 150) public int sceMpegAvcDecodeDetailIndex(int mpeg, int index, @BufferInfo(lengthInfo=pspsharp.HLE.BufferInfo.LengthInfo.fixedLength, Length=52, usage=pspsharp.HLE.BufferInfo.Usage.out) pspsharp.HLE.TPointer32 detail)
		...
		public virtual int sceMpegAvcDecodeDetailIndex(int mpeg, int index, TPointer32 detail)
		{
			...
			//if (log.DebugEnabled)
			{
				System.Console.WriteLine(string.Format("sceMpegAvcDecodeDetailIndex returning width={0:D}, height={1:D}", detail.getValue(8), detail.getValue(12)));
			}
...
//ORIGINAL LINE: @HLEFunction(nid = 0xCF3547A2, version = 150) public int sceMpegAvcDecodeDetail2(int mpeg, @BufferInfo(usage=pspsharp.HLE.BufferInfo.Usage.out) pspsharp.HLE.TPointer32 detail)
		...
		public virtual int sceMpegAvcConvertToYuv420(int mpeg, TPointer yuv420Buffer, TPointer yCbCrBuffer, int unknown2)
		{
			...
			//if (log.DebugEnabled)
			{
				System.Console.WriteLine(string.Format("sceMpegAvcConvertToYuv420 from 0x{0:X8}-0x{1:X8} to 0x{2:X8}-0x{3:X8}", yCbCrBuffer.Address, yCbCrBuffer.Address + size, yuv420Buffer.Address, yuv420Buffer.Address + size));
			}
...
//ORIGINAL LINE: @HLEUnimplemented @HLEFunction(nid = 0xAF26BB01, version = 600) public int sceMpegAvcResourceGetAvcEsBuf()
		...
		public virtual int sceMpegAvcResourceGetAvcEsBuf()
		{
			if (avcEsBuf == null)
			{
				System.Console.WriteLine(string.Format("sceMpegAvcResourceGetAvcEsBuf avcEsBuf not allocated"));
				return -1;
			}

			//if (log.DebugEnabled)
			{
				System.Console.WriteLine(string.Format("sceMpegAvcResourceGetAvcEsBuf returning 0x{0:X8}", avcEsBuf.addr));
			}
