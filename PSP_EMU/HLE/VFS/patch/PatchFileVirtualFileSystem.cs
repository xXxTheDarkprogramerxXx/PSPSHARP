	...
	public class PatchFileVirtualFileSystem : AbstractProxyVirtualFileSystem
	{
		...
		private class PatchInfo
		{
			...
			protected internal virtual void apply(sbyte[] buffer, int offset)
			{
				if (offset >= 0 && offset < buffer.Length)
				{
					...
					{
						System.Console.WriteLine(string.Format("Patching of file '{0}' failed at offset 0x{1:X8}, 0x{2:X8} found instead of 0x{3:X8}", fileName, offset, checkValue, oldValue));
					}
					else
					{
						//if (log.DebugEnabled)
						{
							System.Console.WriteLine(string.Format("Patching file '{0}' at offset 0x{1:X8}: 0x{2:X8} -> 0x{3:X8}", fileName, offset, oldValue, newValue));
						}
			...
			protected internal virtual void patch16(sbyte[] buffer, int offset, int oldValue, int newValue)
			{
				if (offset >= 0 && offset < buffer.Length)
				{
					...
					if (checkValue != oldValue)
					{
						System.Console.WriteLine(string.Format("Patching of file '{0}' failed at offset 0x{1:X8}, 0x{2:X4} found instead of 0x{3:X4}", fileName, offset, checkValue, oldValue));
					}
					else
					{
						//if (log.DebugEnabled)
						{
							System.Console.WriteLine(string.Format("Patching file '{0}' at offset 0x{1:X8}: 0x{2:X4} -> 0x{3:X4}", fileName, offset, oldValue, newValue));
						}
		...
		private class PrxPatchInfo : PatchInfo
		{
			...
			internal virtual int getFileOffset(sbyte[] buffer)
			{
				...
				for (int i = 0; i < phNum; i++)
				{
					...
					if (segmentOffset < 0)
					{
						System.Console.WriteLine(string.Format("Patching of file '{0}' failed: incorrect offset 0x{1:X8} outside of program header segment #{2:D}", fileName, offset, i));
						return -1;
					}
				}

				System.Console.WriteLine(string.Format("Patching of file '{0}' failed: incorrect offset 0x{1:X8} outside of all program header segments", fileName, offset));
				return -1;
			}
				...
				while (o < buffer.Length)
				{
					...
					else
					{
						...
						if (R_BASE == offset)
						{
							...
							{
								System.Console.WriteLine(string.Format("Unsupported relocation patch at 0x{0:X8}, R_FLAG=0x{1:X}", R_BASE, R_FLAG));
								return;
							}
							...
							{
								System.Console.WriteLine(string.Format("Unsupported relocation patch at 0x{0:X8}, R_CMD=0x{1:X4}, nextCmd=0x{2:X4}", R_BASE, R_CMD, nextCmd));
								return;
							}
			...
			public override void apply(sbyte[] buffer)
			{
				...
				if (fileOffset >= 0)
				{
					//if (log.DebugEnabled)
					{
						System.Console.WriteLine(string.Format("Patching file '{0}' at PRX offset 0x{1:X8}: 0x{2:X8} -> 0x{3:X8}", fileName, offset, oldValue, newValue));
					}
		...
		protected internal class PrxSyscallPatchInfo : PrxPatchInfo
		{
			...
			public override void apply(sbyte[] buffer)
			{
				//if (log.DebugEnabled)
				{
					System.Console.WriteLine(string.Format("Patching file '{0}' at PRX offset 0x{1:X8}: {2}", fileName, offset, functionName));
				}
				base.apply(buffer);
