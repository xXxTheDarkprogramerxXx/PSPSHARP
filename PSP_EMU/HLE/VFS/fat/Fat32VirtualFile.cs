...
//ORIGINAL LINE: sealed override int fatSectors = getFatSectors(totalSectors, getSectorsPerCluster());
		...
		protected internal override void writeFatSector(int fatIndex)
		{
			...
			//if (log.DebugEnabled)
			{
				System.Console.WriteLine(string.Format("Fat32VirtualFile.writeFatSector fatIndex=0x{0:X}, offset=0x{1:X}", fatIndex, offset));
			}
