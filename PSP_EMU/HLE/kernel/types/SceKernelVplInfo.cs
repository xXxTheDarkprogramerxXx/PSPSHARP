...
//	import static pspsharp.HLE.kernel.managers.VplManager.PSP_VPL_ATTR_ADDR_HIGH;
	...
	public class SceKernelVplInfo : pspAbstractMemoryMappedStructureVariableLength
	{
		...
		public static SceKernelVplInfo tryCreateVpl(string name, int partitionid, int attr, int size, int memType)
		{
			...
			{
				VplManager.System.Console.WriteLine(string.Format("tryCreateVpl not enough free mem (want={0:D} ,free={1:D}, diff={2:D})", totalVplSize, maxFreeSize, totalVplSize - maxFreeSize));
			}
		...
		public virtual bool free(int addr)
		{
			if (!dataBlockMap.ContainsKey(addr))
			{
				...
				{
					VplManager.System.Console.WriteLine(string.Format("Free VPL 0x{0:X8} address not allocated", addr));
				}
			...
			if (top != allocAddress)
			{
				VplManager.System.Console.WriteLine(string.Format("Free VPL 0x{0:X8} corrupted header", addr));
				return false;
			}
			...
			if (VplManager.log.DebugEnabled)
			{
				VplManager.System.Console.WriteLine(string.Format("Free VPL: Block 0x{0:X8} with size={1:D} freed", addr, deallocSize));
			}
