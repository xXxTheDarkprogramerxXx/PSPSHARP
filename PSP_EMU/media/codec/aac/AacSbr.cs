...
//ORIGINAL LINE: sealed override int delta = (ch && sbr.bsCoupling ? 1 : 0) + 1;
			int delta = (ch && sbr.bsCoupling ? 1 : 0) + 1;
//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'sealed override':
//ORIGINAL LINE: sealed override int odd = sbr.n[1] & 1;
			int odd = sbr.n[1] & 1;
	...
	case 192000:
		...
		private static void sbrMakeFTablelim(SpectralBandReplication sbr)
		{
			...
			{
//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'sealed override':
//ORIGINAL LINE: sealed override float limBandsPerOctaveWarped = bands_warped[sbr.bsLimiterBands - 1];
...
//ORIGINAL LINE: private static void sbrQmfSynthesis(pspsharp.media.codec.util.FFT mdct, float out[], float X[][][], float mdctBuf[], float v0[], int vOff[], sealed override int div)
		private static void sbrQmfSynthesis(FFT mdct, float[] @out, float[][][] X, float[] mdctBuf, float[] v0, int[] vOff, int div)
		{
//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'sealed override':
//ORIGINAL LINE: sealed override float sbrQmfWindow[] = div != 0 ? sbr_qmf_window_ds : sbr_qmf_window_us;
			float[] sbrQmfWindow = div != 0 ? sbr_qmf_window_ds : sbr_qmf_window_us;
//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'sealed override':
//ORIGINAL LINE: sealed override int step = 128 >> div;
			int step = 128 >> div;
			int v;
			int outOffset = 0;
			for (int i = 0; i < 32; i++)
...
//JAVA TO C# CONVERTER WARNING: 'sealed override' parameters are not available in .NET:
//ORIGINAL LINE: private static int sbrHfGen(Context ac, SpectralBandReplication sbr, float Xhigh[][][], sealed override float Xlow[][][], sealed override float alpha0[][], sealed override float alpha1[][], sealed override float bwArray[], sealed override int tEnv[], int bsNumEnv)
		private static int sbrHfGen(Context ac, SpectralBandReplication sbr, float[][][] Xhigh, float[][][] Xlow, float[][] alpha0, float[][] alpha1, float[] bwArray, int[] tEnv, int bsNumEnv)
		{
			...
			for (int j = 0; j < sbr.numPatches; j++)
			{
				...
				{
//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'sealed override':
//ORIGINAL LINE: sealed override int p = sbr.patchStartSubband[j] + x;
		...
		private static int sbrMapping(Context ac, SpectralBandReplication sbr, SBRData chData, int[] eA)
		{
			...
			{
//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'sealed override':
//ORIGINAL LINE: sealed override int ilim = sbr.n[chData.bsFreqRes[e + 1]];
				...
				for (int i = 0; i < sbr.n[1]; i++)
				{
					...
					{
//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'sealed override':
//ORIGINAL LINE: sealed override int m_midpoint = (sbr.fTablehigh[i] + sbr.fTablehigh[i + 1]) >> 1;
		...
		private static void sbrEnvEstimate(float[][] eCurr, float[][][] Xhigh, SpectralBandReplication sbr, SBRData chData)
		{
			...
			if (sbr.bsInterpolFreq)
			{
				...
				{
//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'sealed override':
//ORIGINAL LINE: sealed override float recipEnvSize = 0.5f / (chData.tEnv[e + 1] - chData.tEnv[e]);
			...
			else
			{
				...
				{
//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'sealed override':
//ORIGINAL LINE: sealed override int envSize = 2 * (chData.tEnv[e + 1] - chData.tEnv[e]);
					int envSize = 2 * (chData.tEnv[e + 1] - chData.tEnv[e]);
					int ilb = chData.tEnv[e] * 2 + ENVELOPE_ADJUSTMENT_OFFSET;
					int iub = chData.tEnv[e + 1] * 2 + ENVELOPE_ADJUSTMENT_OFFSET;
//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'sealed override':
//ORIGINAL LINE: sealed override int table[] = chData.bsFreqRes[e + 1] != 0 ? sbr.fTablehigh : sbr.fTablelow;
					int[] table = chData.bsFreqRes[e + 1] != 0 ? sbr.fTablehigh : sbr.fTablelow;
					...
					for (int p = 0; p < sbr.n[chData.bsFreqRes[e + 1]]; p++)
					{
						float sum = 0.0f;
//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'sealed override':
//ORIGINAL LINE: sealed override int den = envSize * (table[p + 1] - table[p]);
		...
		/// </summary>
//JAVA TO C# CONVERTER WARNING: 'sealed override' parameters are not available in .NET:
//ORIGINAL LINE: private static void sbrGainCalc(SpectralBandReplication sbr, SBRData chData, sealed override int eA[])
		private static void sbrGainCalc(SpectralBandReplication sbr, SBRData chData, int[] eA)
		{
			...
			for (e = 0; e < chData.bsNumEnv; e++)
			{
				...
				for (k = 0; k < sbr.nLim; k++)
				{
					...
					{
//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'sealed override':
//ORIGINAL LINE: sealed override float temp = sbr.eOrigmapped[e][m] / (1.0f + sbr.qMapped[e][m]);
...
//JAVA TO C# CONVERTER WARNING: 'sealed override' parameters are not available in .NET:
//ORIGINAL LINE: private static int sbrLfGen(SpectralBandReplication sbr, float Xlow[][][], sealed override float W[][][][], int bufIdx)
		...
		/// </summary>
//JAVA TO C# CONVERTER WARNING: 'sealed override' parameters are not available in .NET:
//ORIGINAL LINE: private static void sbrHfInverseFilter(float alpha0[][], float alpha1[][], sealed override float Xlow[][][], int k0)
...
//JAVA TO C# CONVERTER WARNING: 'sealed override' parameters are not available in .NET:
//ORIGINAL LINE: private static void sbrHfAssemble(float Y1[][][], sealed override float Xhigh[][][], SpectralBandReplication sbr, SBRData chData, sealed override int eA[])
		private static void sbrHfAssemble(float[][][] Y1, float[][][] Xhigh, SpectralBandReplication sbr, SBRData chData, int[] eA)
		{
//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'sealed override':
//ORIGINAL LINE: sealed override int h_SL = 4 * (!sbr.bsSmoothingMode ? 1 : 0);
			int h_SL = 4 * (!sbr.bsSmoothingMode ? 1 : 0);
//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'sealed override':
//ORIGINAL LINE: sealed override int kx = sbr.kx[1];
			int kx = sbr.kx[1];
//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'sealed override':
//ORIGINAL LINE: sealed override int m_max = sbr.m[1];
			int m_max = sbr.m[1];
...
//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'sealed override':
//ORIGINAL LINE: sealed override float g_filt_tab[] = new float[48];
			float[] g_filt_tab = new float[48];
//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'sealed override':
//ORIGINAL LINE: sealed override float q_filt_tab[] = new float[48];
			...
			for (int e = 0; e < chData.bsNumEnv; e++)
			{
				for (int i = 2 * chData.tEnv[e]; i < 2 * chData.tEnv[e + 1]; i++)
				{
					...
					if (h_SL != 0 && e != eA[0] && e != eA[1])
					{
						...
						{
//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'sealed override':
//ORIGINAL LINE: sealed override int idx1 = i + h_SL;
							int idx1 = i + h_SL;
							gFilt[m] = 0.0f;
							qFilt[m] = 0.0f;
		...
		/// Generate the subband filtered lowband
//JAVA TO C# CONVERTER WARNING: 'sealed override' parameters are not available in .NET:
//ORIGINAL LINE: private static int sbrXGen(SpectralBandReplication sbr, float X[][][], sealed override float Y0[][][], sealed override float Y1[][][], sealed override float Xlow[][][], int ch)
		private static int sbrXGen(SpectralBandReplication sbr, float[][][] X, float[][][] Y0, float[][][] Y1, float[][][] Xlow, int ch)
		{
			int k, i;
			const int i_f = 32;
//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'sealed override':
//ORIGINAL LINE: sealed override int i_Temp = Math.max(2*sbr.data[ch].tEnvNumEnvOld - i_f, 0);
