...
//ORIGINAL LINE: private int readByteHexTo10(pspsharp.filesystems.umdiso.UmdIsoFile file) throws java.io.IOException
		...
		private void parsePlaylistFile()
		{
			try
			{
				...
				{
					Console.WriteLine(string.Format("Accessing valid PLAYLIST.UMD file: playListSize={0:D}, playListTracksNum={1:D}", playListSize, playListTracksNum));
				}
...
//ORIGINAL LINE: sealed override String resourceNames[] = new String[] { "100000", "000000", "110000", "010000" };
		...
		private bool goToMpsStream(int streamIndex)
		{
			...
			Console.WriteLine("Loading stream: " + fileName);
			try
			{
				...
				if (cpiFile != null)
				{
					Console.WriteLine("Found CLIPINF data for this stream: " + cpiFileName);
				}
			}
			catch (FileNotFoundException)
			{
