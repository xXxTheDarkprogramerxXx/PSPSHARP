...
//	import static org.objectweb.asm.tree.AbstractInsnNode.TABLESWITCH_INSN;
	...
	/// based on a list of FieldInfo values dynamically computed at runtime.
	/// The specialized class contains only the part of the code that will be
	/// executed for the given FieldInfo values, without the overhead for testing these values.
	/// 
	...
	///    }
	/// 
	/// A specialized class for the following FieldInfo values:
	///    testValue = 123;
	/// would be
	...
	/// - switch
	/// - while
	/// on FieldInfo values of the following types:
	/// - int
	/// - byte
	/// - short
	/// - bool
...
//ORIGINAL LINE: for (java.util.ListIterator<?> lit = fields.listIterator(); lit.hasNext();)
				...
				{
					FieldNode FieldInfo = (FieldNode) lit.Current;
					if ((FieldInfo.access & Opcodes.ACC_STATIC) != 0 && variables.ContainsKey(FieldInfo.name))
					{
